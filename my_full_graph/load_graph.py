...
def prepare_data_2(g, n_classes, args, device):
	...
	# data = g,  feats, labels, n_classes, train_mask, val_mask, test_mask
...
def load_ogbn_dataset_full(name,  args):
	"""
	...
	if name not in ['ogbn-arxiv',"ogbn-products", "ogbn-mag","ogbn-papers100M"]:
		raise RuntimeError("Dataset {} is not supported".format(name))
	...
	if "arxiv" in name:
		...
		return g, nfeats, labels, n_classes, train_nid, val_nid, test_nid, evaluator,splitted_idx
...
def load_ogb(name):
	...
	data = DglNodePropPredDataset(name="ogbn-products", root=os.path.join(home_dir, "graph_partition_multi_layers/benchmark_full_graph", "dataset"))
