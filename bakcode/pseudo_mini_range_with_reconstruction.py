...
def run(args, device, data):
	...
	for epoch in range(args.num_epochs):
		...
		# file_name=r'/DATA/re/fan_out_'+args.fan_out+'/'+args.dataset+'_'+str(epoch)+'_dataloader_.pickle'
		...
		# #----------------------------------------------------------------------------------------------------
		# for step, (_, _, blocks_) in enumerate(full_block_dataloader):
		# 	print(str(epoch) + '  epoch,   batch ' +str(step)+' blocks edges')
		# 	print('full batch blocks: ')
		# 	blocks_[0].edata[dgl.EID],(_,_)=get_global_graph_edges_ids(g, blocks_[0])
		# 	blocks_[1].edata[dgl.EID],(_,_)=get_global_graph_edges_ids(g, blocks_[1])
		# 	batch_inputs, batch_labels = load_block_subtensor(feats, labels, blocks_, device)
		# 	blocks_ = [block.int().to(device) for block in blocks_]
		# 	# Compute loss and prediction
		# 	# see_memory_usage("----------------------------------------before batch_pred = model(blocks, batch_inputs) ")
		# 	batch_pred = model(blocks_, batch_inputs)
		#----------------------------------------------------------------------------------------------------
		...
		# return
		#---------------------------------------------------------------
		# re_block_dataloader=reconstruct_subgraph(g,block_dataloader, full_block_dataloader)
		# re_block_dataloader=reconstruct_subgraph_manually(g,block_dataloader, full_block_dataloader)
		# print(re_block_dataloader!=block_dataloader)
		# return
		# #---------------------------------------------------------------
		# weights_list=[1.0] # 
		# weights_list_compare.append(weights_list)
		...
		#---------------------------------------------------------------------------------------------------------------------	
		# for step, (input_nodes, seeds, blocks) in enumerate(full_block_dataloader):
			# weights_list=[1.0] # 
		# for step, (input_nodes, seeds, blocks) in enumerate(re_block_dataloader):
		# 	weights_list=[1.0] # 
		for step, (input_nodes, seeds, blocks) in enumerate(block_dataloader):
			# blocks=full_batch_blocks
			print(str(epoch) + '  epoch,   batch ' +str(step)+' blocks edges')
			print('reconstruct block : ')
