...
def generate_dataloader(raw_graph, block_to_graph_list, args):
	...
	if 'partition' in args.selection_method:
		...
		# dataloader, weights_list,time_collection  = generate_dataloader_wo_gp_Pure_range(raw_graph, block_to_graph_list, args)
		# reconstruct_graphs_list=reconstruct_blocks_to_subgraph(raw_graph, dataloader)
		# print(reconstruct_graphs_list)
...
# def reconstruct_blocks_to_subgraph(raw_graph, block_dataloader):
# 	layer_0=[]
# 	layer_1=[]
# 	layer_2=[]

# 	for step, (input_nodes, seeds, blocks) in enumerate(block_dataloader):
# 		print( '  batch ' +str(step)+' blocks edges')
...
# 	for i, bi in enumerate(layer_0):
# 		print('layer_0    block : ',i )
# 		print(bi.srcdata[dgl.NID].tolist())
# 		print(bi.dstdata[dgl.NID].tolist())
# 		print(bi.edata[dgl.NID].tolist())
# 		print(bi.edges()[0].tolist())
# 		print(bi.edges()[1].tolist())
# 		print('%'*50)
# 		dst_0.append(bi.dstdata[dgl.NID].tolist())
# 		src_0.append(bi.srcdata[dgl.NID].tolist())
# 		eids_0.append(bi.edata[dgl.NID].tolist())
	
# 	eids=sort_list_(eids_0)
# 	dst_layer_0=torch.tensor(sort_list_(dst_0))
# 	print(sorted(sort_list_(dst_0)))
	
# 	layer_0_graph=dgl.edge_subgraph(raw_graph,eids,store_ids=True)
# 	block_layer_0 = dgl.to_block(layer_0_graph, dst_nodes=dst_layer_0)
# 	for i, bi in enumerate(layer_1):
# 		print('layer_1    block : ',i )
# 		print(bi.srcdata[dgl.NID].tolist())
# 		print(bi.dstdata[dgl.NID].tolist())
# 		print(bi.edata[dgl.NID].tolist())
# 		print(bi.edges()[0].tolist())
# 		print(bi.edges()[1].tolist())
# 		print('%'*50)
# 		dst_1.append(bi.dstdata[dgl.NID].tolist())
# 		src_1.append(bi.srcdata[dgl.NID].tolist())
# 		eids_1.append(bi.edata[dgl.NID].tolist())
# 	return
...
def generate_dataloader_wo_gp_Pure_range_block(raw_graph, full_block_dataloader, args):
	...
	#b it means the graph partition starts 
	# from the output layer to the first layer input block graphs.
	for _,(src_full, dst_full, blocks) in enumerate(full_block_dataloader): # only one full batch blocks
		
		reversed_blocks=Reverse(blocks)
		l=len(reversed_blocks)
