...
def run(args, device, data):
	...
	while True:
		...
		for epoch in range(args.num_epochs):
			...
			# file_name_1=r'/DATA/re/fan_out_'+args.fan_out+'/'+args.dataset+'_'+str(epoch)+'_dataloader_.pickle'
			
			# with open(cwd+file_name_1, 'wb') as handle:
			# 	pickle.dump(full_batch_dataloader, handle, protocol=pickle.HIGHEST_PROTOCOL)
			...
			for full_batch_step, (input_nodes, output_seeds, full_batch_blocks) in enumerate(full_batch_dataloader):
				...
				with open(cwd+file_name, 'wb') as handle:
					pickle.dump((input_nodes, output_seeds, full_batch_blocks), handle, protocol=pickle.HIGHEST_PROTOCOL)
				# for layer, cur_block in enumerate(full_batch_blocks):
				# 	file_name=r'/DATA/re/fan_out_'+args.fan_out+'/'+args.dataset+'_'+str(epoch)+'_Block_'+str(layer)+'.pickle'
				# 	with open(cwd+file_name, 'wb') as handle:
				# 		pickle.dump((input_nodes, output_seeds, cur_block), handle, protocol=pickle.HIGHEST_PROTOCOL)
		...
		if args.fan_out=='10,25':
			if args.dataset=='karate' and abs((numpy.mean(avg_src) - 34) )< 2:     ### 10,25
				return
			if args.dataset=='cora' and abs((numpy.mean(avg_src) - 1362)) < 5:     ### 10,25
				return
