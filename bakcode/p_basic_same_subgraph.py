...
def run(args, device, data):
	...
	for epoch in range(args.num_epochs):
		...
		for i in range(args.num_layers):
			file_name='./DATA/re/fan_out_'+args.fan_out+'/'+args.dataset+'_'+str(epoch)+'_Block_'+str(i)+'.pickle'

			# Load data (deserialize)
			with open(file_name, 'rb') as handle:
				block = pickle.load(handle)
			# check_graph_equal(block_layer_0, block, check_feature=True)
			# print('after checking')
			cur_subgraphs.insert(0,block)
		full_batch_sub_graph_data_list.append()
		...
		# 	cur_subgraphs.insert(0,_subgraph[0][0])
		# 	current_block_eidx, current_block_edges = get_global_graph_edges_ids_2(g, cur_subgraph)
		# 	cur_subgraph.edata['_ID'] = current_block_eidx
	...
	in_collection_t=[]
	for epoch, full_batch_subgraphs in enumerate(full_batch_sub_graph_data_list): # args.epochs
		print('Epoch ' + str(epoch))		
		# data loader sampling fan-out neighbor each new epoch
		
		tic = time.time()
		block_dataloader, weights_list,time_collection = generate_dataloader_block(g, full_batch_subgraphs, args)
		# block_dataloader, weights_list,time_collection = generate_dataloader(g, full_batch_subgraphs, args)
